...
# ... (start_experiment, list_experiments vb. diğer tüm fonksiyonlar aynı kalıyor) ...
...
def _generate_config_combinations(config: Dict[str, Any]) -> Generator[Dict[str, Any], None, None]:
    ...
    for combo_values in itertools.product(*param_values):
        ...
        for i, key_path in enumerate(param_names):
            keys, d = key_path.split('.'), new_config
...
def start_experiment(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if batch_id is None: batch_name = None
    for single_config in combinations:
        single_config['batch_id'], single_config['batch_name'] = batch_id, batch_name
...
def get_experiment_report_path(experiment_id: str) -> str:
    ...
    try:
        ...
        # 'experiment_dir' konfigürasyon içinden alınır.
        report_dir = exp.config.get('experiment_dir')
        if not report_dir or not os.path.isdir(report_dir):
            raise AzuraForgeException(status_code=404, detail=f"Report directory for experiment '{experiment_id}' not found.", error_code="REPORT_NOT_FOUND")
...
def _get_pipeline_instance(exp: Experiment) -> TimeSeriesPipeline:
    exp_id = exp.id
    if exp_id in _pipeline_cache: return _pipeline_cache[exp_id]
    from azuraforge_worker.tasks.training_tasks import AVAILABLE_PIPELINES
    pipeline_name = exp.pipeline_name
    if pipeline_name not in AVAILABLE_PIPELINES: raise PipelineNotFoundException(pipeline_id=pipeline_name)
...
def predict_with_model(experiment_id: str, request_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    try:
        ...
        model_input_shape = (1, pipeline_instance.config.get('model_params', {}).get('sequence_length', 60), len(pipeline_instance.feature_cols))
